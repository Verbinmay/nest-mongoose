...
var HttpExceptionFilter = /** @class */ (function () {
    ...
    HttpExceptionFilter.prototype["catch"] = function (exception, host) {
        ...
        if (status === 400) {
            ...
            var expResponse = exception.getResponse();
            expResponse.message.forEach(function (m) { return errorResponse_1.errors.push(m); });
            response.status(status).json(errorResponse_1);
        }
        else {
